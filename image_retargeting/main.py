...
from image_retargeting.utils import get_file_name, SyntesisConfigurations
...
def generate_texture():
    for texture_image_path in [
        ...
        criteria = losses.PatchMMD_RBF(patch_size=7, stride=1, normalize_patch='none')
        # criteria = losses.PatchSWDLoss(patch_size=11, stride=1, num_proj=512, normalize_patch='none')
...
def style_transfer():
    for style_image_path, content_image_path in [
        ...
        criteria = losses.PatchSWDLoss(patch_size=13, stride=1, num_proj=512, normalize_patch='none')
        # criteria = losses.VGGPerceptualLoss(pretrained=True, features_metric_name='gram',
...
def image_resampling():
    ...
    for input_image_path in [
        ...
        for i in range(3):
            ...
            criteria = losses.PatchSWDLoss(patch_size=11, stride=1, num_proj=256, normalize_patch='none')
...
def image_retargeting():
    for input_image_path in [
        ...
        for aspect_ratio in [(1, 0.9)]:
            # criteria = losses.MMDApproximate(patch_size=11, strides=1, sigma=0.03, pool_size=-1, r=512, normalize_patch='channel_mean')
            criteria = losses.PatchSWDLoss(patch_size=7, stride=1, num_proj=1024, normalize_patch='channel_mean')

            conf = SyntesisConfigurations(pyr_factor=0.75, n_scales=5, aspect_ratio=aspect_ratio, lr=0.05, num_steps=500, init="blur", resize=256, blur_loss=0)
            ...
            retarget_image(input_image_path, criteria, content_loss, conf, outputs_dir)
