...
#     def __init__(self, patch_size=3, strides=1, rbf_sigma=0.06, batch_reduction='mean', use_keops=True, normalize_patch='none'):
#         super().__init__()
...
#         L = x_uf.shape[1]
#         total_loss = self.pooler(x_uf.reshape(x_bs * L, *window_shape), y_uf.reshape(y_bs * L, *window_shape)).view(x_bs, L).mean(dim=1)
