...
def apply_ditribution_metric_to_images(x, y, dist_metric, patch_size, stride, n_samples=None, sample_same_locations=True, normalize_patch='mean'):
    ...
    # patches are of size (b, num_patches, k x k x 3)
    ...
    elif y_patches.shape[1] > x_patches.shape[1]:
...
class PatchdistributionsLoss(torch.nn.Module):
    def __init__(self, dist_metric, patch_size=7, stride=1, n_samples=None, sample_same_locations=True, batch_reduction='mean', normalize_patch='none'):
        """Splits each pair of images pair to patches and apply a metric on the two sets of patches"""
    ...
    def forward(self, x, y):
        results = apply_ditribution_metric_to_images(x, y, self.dist_metric,
