...
def apply_ditribution_metric_to_patches(x, y, dist_metric, patch_size=7, stride=1, n_samples=None, sample_same_locations=True):
...
class PatchdistributionsLoss(torch.nn.Module):
    def __init__(self, dist_metric, patch_size=7, stride=1, n_samples=None, sample_same_locations=True, batch_reduction='mean'):
    ...
    def forward(self, x, y):
        results = apply_ditribution_metric_to_patches(x, y, self.dist_metric, patch_size=self.patch_size, stride=self.stride, n_samples=self.n_samples)
        # results *= 1e3
