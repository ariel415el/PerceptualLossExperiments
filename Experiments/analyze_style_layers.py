...
def plot_cov_matrix(image, layer_name, loss_network, save_path):
    with torch.no_grad():
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        image_activations = loss_network.get_activations(image)

        fig = plt.figure()
        c, h, w = image_activations[layer_name][0].size()
        vecs = image_activations[layer_name][0].view(c, h * w).cpu().numpy()
        # mat = np.cov(vecs)
...
def plot_gram_matrix(image, layer_name, loss_network, save_path):
    with torch.no_grad():
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        image_activations = loss_network.get_activations(image)

        mat = gram(image_activations[layer_name]).cpu().numpy()
        plt.matshow(mat[0])
        plt.savefig(save_path)
        plt.clf()
...
def run_batch_experiments():
    ...
    features_metric = get_features_metric('gram_trace')

    for image_path in images:
        image_name = os.path.basename(image_path).split('.')[0]
        image = imload(image_path, imsize=128).to(device)
        for loss_network in losses:
            weights_type = 'PT' if loss_network.pretrained else 'rabdom'
            for layers_set in [['relu1_2', 'relu2_2'], ['relu1_2', 'relu2_2', 'relu3_3']]:
                save_path = f"{train_dir}/gram_minimization/{image_name}-{weights_type}-{layers_set}.png"
            ...
            for layer_name in layers_set:
                save_path = f"{train_dir}/corr_mats/{image_name}-{weights_type}-{layer_name}.png"
                plot_cov_matrix(image, layer_name, loss_network, save_path)
                save_path = f"{train_dir}/gram_mats/{image_name}-{weights_type}-{layer_name}.png"
